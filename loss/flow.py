...
class EventWarping(torch.nn.Module):
    ...
    def event_flow_association(self, flow, event_list, pol_mask, event_mask):
        """
        :param flow: [batch_size x 2 x H x W] optical flow (x, y) maps
        ...
        flow = flow.view(B, 2, H, W)
        if len(self._flow_list) == 0:
         self._flow_list.append(event_flow)
        else:
            self._flow_list[0] = torch.cat([self._flow_list[0], event_flow], dim=1)  
        ...
        if len(self._flow_maps_x)==0:
            self._flow_maps_x.append(flow[:, 0:1, :, :])
            self._flow_maps_y.append(flow[:, 1:2, :, :])         
        else:
            self._flow_maps_x[0] = torch.cat([self._flow_maps_x[0], flow[:, 0:1, :, :]], dim=1)
            self._flow_maps_y[0] = torch.cat([self._flow_maps_y[0], flow[:, 1:2, :, :]], dim=1)
