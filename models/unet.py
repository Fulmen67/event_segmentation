...
class MultiResUNet_Flow(BaseUNet):
    """
    UNet architecture for EV-FlowSegNet
    """
    ...
    def build_decoders_segmentation_module(self):
        ...
        decoder_output_sizes.append(self.num_motion_models)
        
        
        decoders = nn.ModuleList()
        for input_size, output_size in zip(decoder_input_sizes, decoder_output_sizes):
            # the last layer of the decoder serves to bring the number of channels from 32 to 5, so that we have a mask
            if input_size == 32 and output_size == self.num_motion_models:
                decoders.append(
                    self.ff_type(
                        input_size,
                        output_size,
    ...
    def build_encoders_optical_flow_module(self):
        ...
        for i, (input_size, output_size) in enumerate(zip(input_list, output_list)):
            """if i == 0:
                input_size = self.num_bins"""
            encoders.append(
                self.ff_type(
        ...
        for i in range(1,len(feature_size)):
            feedforward.append(
                self.linear_type(
                    feature_size[i-1],
                    feature_size[i],  
                    activation= None if i == len(feature_size)-1 else "tanh"
                )
            )
    ...
    def forward(self, x):
        ...
        #flatten x_flow
        x_flow = x_flow.view(x_flow.shape[0], -1)
        ...
        return {'motion models': x_flow}
    
class MultiResUNet_Segmentation(BaseUNet):
    """
    UNet architecture for EV-LayerSegNet
    """
    ...
    def forward(self, x):
        ...
        return {'motion models': x_flow, 'alpha mask': x_seg}
